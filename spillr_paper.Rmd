...
combo <- combo %>% select(all_of(c(ch, "method")))
combo$method <- factor(combo$method, 
                       levels = c("uncorrected", "CATALYST", "spillR"))

p_before_after <- combo %>% 
  filter(.data[[ch]] > 0) %>%
  ggplot(aes(.data[[ch]], color = method, linetype = method)) + 
  geom_freqpoly(alpha = 1.0, bins = 50, linewidth = 0.8) +
  xlab(ch_name) +
  ggtitle("Spillover Compensation on Real Cells (Only Positive Counts)") +
  xlim(x_lim)

# --------- spillover probability curves ---------

tb_spill_prob <- metadata(sce_spillr)$spillover_est[[ch_metal]]
tb_spill_prob$k <- as.factor(runmed_k)
...
# --------- combine everything ---------
plot_grid(
  p_beads, p_before_after, p_spill, 
