...
The experiment for [(A1)](#assumption1) shows that the mean count after `spillR` correction is closer to the true mean over a wide range of bead shifts. This indicates that our methods works for even low quality bead experiments. The experiment for [(A2)](#assumption2) shows that our methods is robust to model misspecification. Moreover, misspecification should not be a major issue as we can add all channels if necessary. The increase in computational cost is relatively minor as our method scales linearly in the number of spillover markers. The experiment for [(A3)](#assumption3) shows that the mean count after correction is still closer to the true mean even with clear violation of this assumption. This justifies our strong parametric assumption on the spillover probability even when the assumption is violated.

In our comparison to `CATALYST` on real data, we observe two difference correction strategies. `CATALYST` essentially shrinks counts towards zero by minimizing a non-negative least squares objective. `CATALYST` assumes that spillover is linear up to counts of 5,000. `spillR` does not require linearity of the spillover, but assumes that the distribution on the beads experiment carries over to the real cells experiment. The applied shrinkage is the same for low counts (e.g., below 10) and high counts (e.g., more than 100). `spillR` does not really on this linearity assumption. If counts are not on the spillover range, then they are not correction no matter how low their values are. If counts are in the spillover range, then we aggressively mask them. But even with this aggressive masking, we do not remove correlations between markers. The marker correlation between HLA-ABC (Yb172Di) and CD3 (Yb173Di) illustrated this point. `CATALYST` removes the positive correlation, whereas `spillR` keeps the correlation for the higher counts. Compensation methods should try to remove spillover while keeping biological meaningful signal. If not, then downstream analysis that rely on these correlations be biased.
...
The main advantage of our method is the diagnostics plot of the spillover probability curve. We can judge if the curve makes sense by comparing it to the observed count and bead distributions. Methods based on non-negative least squares are harder to diagnose as they minimize a cost function with no clear biological interpretation.
