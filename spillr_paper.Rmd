...
row_names <- rownames(counts_mat)
real_vec <- rpois(n = length(counts_mat), lambda = 1150)
bead_vec <- rpois(n = length(counts_mat), lambda = 39)
spill_vec <- rbinom(n = length(counts_mat), size = 1, prob = 0.5)
counts_vec <- (1-spill_vec) * real_vec + spill_vec * bead_vec
counts_mat <- counts_vec |> 
  matrix(nrow = nrow(counts_mat), ncol = ncol(counts_mat))
rownames(counts_mat) <- row_names
assay(sce, "counts") <- counts_mat
assay(sce, "exprs") <- tfm(counts_mat)
...
x_lim <- c(0, 7)
plot_grid(
  ps[[1]] + xlim(x_lim) + geom_vline(xintercept = c(tfm(39), tfm(1150))),
  ps[[2]] + xlim(x_lim) + geom_vline(xintercept = c(tfm(39), tfm(1150))),
  ncol = 1, align = "v"
)
```
