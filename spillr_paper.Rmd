...
Here we illustrate the procedure using a numerical example that includes one target and one spillover marker. We have data vectors $\mathbf{y}$ and $\mathbf{X}$,
$$
...
\end{bmatrix}.
$$
The first vector represents the counts for the target marker measured in the experiment with real cells, and the second vector contains counts from the beads experiment. In practice, the second vector is usually a matrix with multiple columns representing multiple spillover markers.
...
$$
and re-assign counts to the marker with largest posterior probability,
$$
