...
\textcolor{red}{Panel B shows a curve representing our spillover probability estimates. We can see that the probability of spillover is high for counts with a high density of spillover markers and a low target marker density in Panel A. If the spillover probability is close to one, our correction step assigns most cells to spillover.} Counts above four stem from spillover with probability zero (and from the actual target with probability one), which means that our procedure keeps them at their raw uncorrected value.
...
\textcolor{red}{events}
from bead experiments is much smaller than from real experiments. For $k\geq 2$, the $k$th column of $\mathbf{Y}$ contains marker counts for a given spillover marker, which represents the empirical spillover distribution of marker $k$ into the target marker represented by the first column of $\mathbf{Y}$.
...
To perform spillover compensation, we draw from a Bernoulli distribution with the spillover probability as parameter to decide whether or not to assign a given count to spillover. \textcolor{red}{We mark counts attributed to spillover by setting them to a user-specified value. We recommend a value of zero to maintain the overall cellular composition of the sample or a value such as \texttt{NA} or $-1$ to enable downstream analyses to treat those values separately (for example, calculating means only over non-spillover counts).}
...
\textcolor{red}{
The goal of the experiment is to estimate the mean of the true counts (black density curve) as accurately as possible from the observed counts (yellow density curve). Using \texttt{NA} imputation for spillover counts, the average of the compensated observed counts when ignoring \texttt{NA} values is the mean of the true counts if all spillover counts are correctly identified as such.
}
...
\textcolor{red}{In all six panels (A--F), we observe that \texttt{spillR} compensates most strongly in the low counts, whereas \texttt{CATALYST} compensates strongly in the middle range. \texttt{CATALYST} performs no visible compensation in the second column. Our \texttt{spillR} methods compensate strongly in the second column. A representative pattern is in panel C, CD3 (Yb173Di) against HLA-ABC (Yb172Di), \texttt{CATALYST} can be seen to compensate strongly in the middle range. It removes the spherical pattern that shows correlation between the two markers. \texttt{spillR} preserves this correlation structure and only masks out the lower counts of CD3 (Yb173Di). This highlights a key difference between \texttt{spillR} and \texttt{CATALYST}: \texttt{spillR} identifies counts that may arise from spillover and replaces them with a user-specified value (e.g. 0, \texttt{NA}, or -1), whereas \texttt{CATALYST} shrinks counts across the entire range to compensate for spillover. \texttt{spillR-naive} compensates most aggressively, assigning a wide range of counts outside the target marker bead distribution to spillover.}
