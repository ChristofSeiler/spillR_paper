...
The dashed curve is our estimated spillover probability. We can see that at three on the horizontal axis the probability is around 0.8. Our correction step assigns around 80% of cells to spillover, and keeps the other 20% at the current value. Counts below two have spillover probability of one, which means that our procedure assigns all cells to spillover. Counts above five stem from spillover with probability 0 (and from the actual target with probability 1), which means that our procedure keeps them at their raw uncorrected value.

## Definition of Spillover Probability and Assumptions

We observe a count $Y_{ij}$ of marker $j$ in cell $i$. If spillover is present, then some observations of $Y_{ij}$ will correspond to counts of the true marker $j$ and others to markers $k \ne j$ spilling over into marker $j$. We model the observed $Y_{ij}$ as a finite mixture [@mclachlan2019finite] of unobserved true marker counts $Y_{ij} \mid Z_{ij} = 1, \dots, Y_{ij} \mid Z_{ij} = K$, which each may spill into $Y_{ij}$ with spillover probability $\pi_{1j}, \dots, \pi_{Kj}$, 
$$
P(Y_{ij} = y) = \sum_{k = 1}^K \pi_{kj} \, P(Y_{ij} = y \mid Z_{ij} = k).
$$
The probability of spillover into itself corresponds to the proportion of true signal in the observed counts. Conceptually, we divide the probabilities into true signal probability, $\pi_{jj}$, and the probability of spillover, that is, the total probability of all but the $j$th probability, $\sum_{k \ne j} \pi_{kj}$. The total of all the spillover probabilities equals to one, $\sum_k \pi_{kj} = 1$. The total number of markers in mass cytometry panels is between 30 and 40 [@bendall2011single]. Typically, only a small subset $K$ of three to four markers spill over into the target marker [@catalyst].
...
* <div id='assumption2'>(A2) Observed counts $Y_{ij}$ can only be due to one single distribution $Y_{ij} \mid Z_{ij} = k$.</div>
...
To parse this calculation, recall that in mixture models the probability $\pi_{jj}$ is the prior probability, $P(Y_{ij} = y \mid Z_{ij} = j)$ is the conditional probability of obtaining count $y$ given the mixture component for marker $j$ spilling over into marker $j$, and the denominator is the marginal distribution. Putting all this together and applying Bayes rule leads to the posterior probability.
...
$$
P^{(0)}(Y_{ij} =y \mid Z_{ij} = k) = \frac{\sum_{i = 1}^{n_k} \mathbf{1}\left(y_{ik}^{(0)} = y\right)}{\sum_{j = 1}^K \sum_{i = 1}^{n_j} \mathbf{1}\left(y_{ij}^{(0)} = y\right)},
$$
...
This step is a post-processing step of $\hat{P}(\text{spillover} \mid Y_{ij} = y)$. Until now, the procedure is fully nonparametric. We apply no regularization to $y$'s that are close to one another. As a result, the spillover probability estimates are noisy and we regularize them using our assumption [(A3)](#assumption3).
...
$$
\tilde{P}(\text{spillover} \mid Y_{ij} = y) = \left(1 + e^{-(\hat{\beta}_0 + \hat{\beta}_1 y)}\right)^{-1}.
$$
...
$$
\begin{aligned}
& P^{(0)}(Y_{i1} = 2 \mid Z_{i1} = 1)  =  0 \quad & \hat{P}(\text{spillover} \mid Y_{ij} = 2) = 1 \\
& P^{(0)}(Y_{i1} = 3 \mid Z_{i1} = 1)  =  0       & \hat{P}(\text{spillover} \mid Y_{ij} = 3) = 1 \\
& P^{(0)}(Y_{i1} = 5 \mid Z_{i1} = 1)  =  1/3     & \hat{P}(\text{spillover} \mid Y_{ij} = 5) = 2/3 \\
& P^{(0)}(Y_{i1} = 17 \mid Z_{i1} = 1) =  2/3     & \hat{P}(\text{spillover} \mid Y_{ij} = 17) = 1/3.
\end{aligned}
$$
