...
\end{cases}
$$
mask counts in vector $\mathbf{y}$ whose posterior probability of belonging to any non-target spillover component is larger than that of belonging to the target component, and re-estimate the distribution for the target marker with the newly masked counts,
$$
...
$$
\begin{aligned}
& P^{(0)}(Y_i = 2 \mid Z_i = 1)  =  1/6 \qquad & P^{(0)}(Y_i = 2 \mid Z_i = 2) = 2/3 \\
& P^{(0)}(Y_i = 3 \mid Z_i = 1)  =  2/6 \qquad & P^{(0)}(Y_i = 3 \mid Z_i = 2) = 1/3 \\
...
$$
and mask (indicated by an $\times$) elements in vector $\mathbf{y}$ whose posterior probability of belonging to marker 2 is larger than that of belonging to marker 1,
$$
