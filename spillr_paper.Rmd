...
\textcolor{red}{
The panels D and F show no compensation by \texttt{CATALYST}. For the same panels our \texttt{spillR} methods compensates quite strongly. Our diagnostic plots using function \texttt{plotDiagnostics} in \texttt{spillR} give some indication on why our method compensates that strongly. The design of the original experiment is such that we expect no spillover in the channels on the vertical axis. But our diagnostic plots show that the bead distributions from spillover marker---as identified by the spillover matrix in \texttt{CATALYST}---exhibit strong overlap between at least one spillover marker and the first mode in the distribution of real cells. Thus, our strong compensation is consistent with our assumption (A1). The behavior of \texttt{spillR} (fast) corroborate our findings by completely removing all counts in that area. This occurs when there is only spillover and no signal from the target marker.
} 
<!-- [(A1)](#assumption1) -->
