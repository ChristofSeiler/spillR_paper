...
compute_average <- function(ftn, parameter) {
  data <- ftn(parameter)
  comp <- compensate(data$df_real, data$df_bead, 
                     target_marker = "Y", 
                     spillover_markers = "Z")
  y_comp  <- comp$tb_compensate$corrected
  y_truth <- data$Z_target
  mean(y_comp, na.rm = TRUE) - mean(y_truth)
}
...
d_a1 <- expand.grid(
  parameter = seq(-30, 0, 1), 
  replicate = 1:n_rep,
  title = "Bead Shift (A1)"
  ) %>%
  rowwise() %>% 
  mutate(w1 = compute_average(generate_data_a1, parameter))
...
# create parameter combination
d_a2 <- expand.grid(
  parameter = seq(from = 0, to = 0.4, by = 0.01), 
  replicate = 1:n_rep,
  title = "Model Misspecification (A2)"
  ) %>%
  rowwise() %>% 
  mutate(w1 = compute_average(generate_data_a2, parameter))
...
# create parameter combination
d_a3 <- expand.grid(
  parameter = seq(from = 0, to = 1, by = 0.02), 
  replicate = 1:n_rep,
  title = "High Count Spillover (A3)"
  ) %>%
  rowwise() %>% 
  mutate(w1 = compute_average(generate_data_a3, parameter))

# --------- combined plot ---------

tb_experiment <- bind_rows(d_a1, d_a2, d_a3)
tb_summary <- tb_experiment %>% 
    group_by(title, parameter) %>% 
    summarize(
      mean = mean(w1),
      se = sd(w1)
      ) %>% 
...
```{r critical-points, fig.height = 5, fig.width = 9, out.width = "90%", fig.align = "center", echo = FALSE, warning = FALSE, message = FALSE, fig.cap="Three points from each experiment."}
generate_critical <- function(fn, parameter) {
  ...
  bind_rows(
    ...
    mutate(parameter = paste0("tau = ", parameter)) %>%
...
(S \mid M = 1)   & \sim \text{Poisson}(130)  & \qquad \text{(second mode of spillover)} \\
