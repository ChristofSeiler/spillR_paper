...
tb_summary <- tb_experiment %>% 
    ...
    summarize(
      ...
      target = mean(mean_truth),
      `spillR corrected` = mean(mean_comp)
      ) %>% 
    ungroup()

p_overview <- pivot_longer(tb_summary, -c(title, tau), 
             names_to = "mean", values_to = "count") %>%
  mutate(mean = factor(mean, levels = c("target", 
                                        "uncorrected", 
                                        "spillR corrected"))) %>%
...
generate_critical <- function(fn, tau) {
  data <- fn(tau)
  bind_rows(
    tibble(count = data$Z_target, variable = "target", type = "truth"),
    tibble(count = data$Z_spill, variable = "spillover", type = "truth"),
    tibble(count = data$df_real$Y, variable = "target", type = "observed"),
