...
We compare `spillR` and `CATALYST` on semi-simulated data. The goal of is to explain some of the differences between `spillR` and `CATALYST` and to evaluate the performance of `spillR` on more than one spillover marker. We create an synthetic beads for CD3 (Yb173Di) by overwriting the beads channel with the true channel from the real experiment. We take the first mode of the true signal in CD3 (Yb173Di) between 10 and 300 counts. We shift these counts (0 is no shift, subtract 0.47 after data transformation, and subtract 0.94 after data transformation). We subsample this new distribution to keep the same number of beads. Figure \@ref(fig:semi-simulated-plot) is the same plot as in the introduction, but now with the synthetically modified beads distribution.
