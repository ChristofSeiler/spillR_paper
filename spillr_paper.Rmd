...
```{r simulated-experiments-plot, fig.height = 7, fig.width = 9, out.width = "90%", fig.align = "center", echo = FALSE, warning = FALSE, message = FALSE, fig.cap="Three experiments testing our three assumptions (A1), (A2), and (A3). Top row: mean values over the entire range of the experimental setups. Second to last row: density plots of three parameter settings."}
...
In all six panels, we observed that `spillR` compensates more aggressively in the low counts. In panel CD3 (Yb173Di) against HLA-ABC (Yb172Di), `CATALYST` compensates more aggressively in the middle range. It removes the spherical pattern that shows correlation between the two markers. `spillR` preserves this correlation structure and only masks out the lower counts of CD3 (Yb173Di). This highlights a key difference between `spillR` and `CATALYST`. We do not correct all counts by shrinking them, but we remove some counts, in the hope that the distribution of the remaining counts that we keep is close to the true distribution. `CATALYST` follows another strategy by shrinking not just low counts but across the entire range.

The color code of the two-dimensional histograms counts the absolute number of cells. The reason why the coloring of the uncorrected and `spillR` corrected plots have a slightly different color code even for the high counts is due rounding. We convert values to the next lower integer.
