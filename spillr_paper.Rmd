...
\textcolor{red}{
We compare our methods to a fast baseline method with the following steps: We estimate the bead PMF of each bead $k$ with the smoother function \texttt{density} in R, $\widehat{P}(Y_i | Z_i = k)$. We normalize the PMF at a specific count value $Y_i = y$ to one and calculate the spillover probability as, 
$$
...
\textcolor{red}{
\texttt{spillR} (fast) compensates most aggressively, removing everything that is the support of the beads distributions.
}
...
\textcolor{red}{
The average computational time with 100 replications on an Apple M1 with 8 cores and 16 GB of RAM are $10.6$ seconds for \texttt{spillR}, $0.43$ seconds for \texttt{CATALYST}, and $0.45$ seconds for \texttt{spillR} (fast).
}
