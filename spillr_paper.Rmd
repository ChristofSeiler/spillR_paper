...
d_a1 <- expand.grid(
  parameter = seq(-60, 0, 1), 
  replicate = 1:n_rep,
  title = "Bead Shift (A1)"
  ) %>%
  rowwise() %>% 
  mutate(w1 = compute_wasserstein(generate_data_a1, parameter))
...
# create parameter combination
d_a2 <- expand.grid(
  parameter = seq(from = 0, to = 0.6, by = 0.01), 
  replicate = 1:n_rep,
  title = "Model Misspecification (A2)"
  ) %>%
  rowwise() %>% 
  mutate(w1 = compute_wasserstein(generate_data_a2, parameter))
...
# create parameter combination
d_a3_bead <- expand.grid(
  parameter = seq(5, 150, 1), 
  replicate = 1:n_rep,
  title = "Bimodal Beads (A3)"
  ) %>% 
  rowwise() %>%
  mutate(w1 = compute_wasserstein(generate_data_a3_bead, parameter))
...
# create parameter combination
d_a3_real <- expand.grid(
  parameter = seq(5, 150, 1), 
  replicate = 1:n_rep,
  title = "Bimodal Real Cells (A3)"
  ) %>% 
  rowwise() %>% 
  mutate(w1 = compute_wasserstein(generate_data_a3_real, parameter))

# --------- combined plot ---------

tb_experiment <- bind_rows(d_a1, d_a2, d_a3_bead, d_a3_real)
