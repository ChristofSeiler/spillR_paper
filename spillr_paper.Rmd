...
We observe a count $Y_{ij}$ of marker $j$ in cell $i$. If spillover is present, then some observations of $Y_{ij}$ will correspond to counts of the true marker $j$ and others to markers $k \ne j$ spilling over into marker $j$. We model the observed $Y_{ij}$ as a finite mixture [@mclachlan2019finite] of unobserved true marker counts $Y_{ij} \mid Z_i = 1, \dots, Y_{ij} \mid Z_i = K$, which each may spill into $Y_{ij}$ with spillover probability $\pi_1, \dots, \pi_K$, 
$$
P(Y_{ij} = y) = \sum_{k = 1}^K \pi_{kj} \, P(Y_{ij} = y \mid Z_{ij} = k).
$$
The probability of spillover of $k$ into itself corresponds to the proportion of true signal in the observed counts. The total of all the spillover probabilities equals to one, $\sum_k \pi_{kj} = 1$. Conceptually, we divide the probabilities into true signal probability, $\pi_{jj}$, and the probability of spillover, that is, the total probability of all but the $j$th probability, $\sum_{k \ne i} \pi_{kj}$. The total number of markers $J$ in mass cytometry panels is between 30 and 40 [@bendall2011single]. Typically, only a small subset $K$ of three to four markers spill over into the target marker [@catalyst].

Experimentally, we only measure a sample from the distribution $Y_{ij}$. The probabilities $\pi_k$ and true distributions $P(Y_{ij} = y \mid Z_{ij} = k)$ are unobserved, and we need to estimate them from data. In many applications, the mixture components, $P(Y_{ij} = y \mid Z_{ij} = 1), \dots, P(Y_{ij} = y \mid Z_{ij} = K)$, are in a parametric family, for example, the negative binomial distribution. Here we propose to fit nonparametric mixture components. We make three assumptions that render the components and mixture probabilities identifiable:
...
* <div id='assumption2'>(A2) Observed counts $Y_{ij}$ can only be due to one single distribution $Y_{ij} = y \mid Z_{ij} = k$.</div>
...
1 - \frac{\pi_{jj} \, P(Y_{ij} = y \mid Z_{ij} = j)}{\sum_{j = 1}^K \pi_{kj} \, P(Y_{ik} = y) \mid Z_{ij} = k)}.
$$

To parse this calculation, recall that in mixture models the probability $\pi_{kj}$ is the prior probability, $P(Y_{ij} = y \mid Z_{ij} = k)$ is the conditional probability of obtaining count $y$ given the mixture component for marker $k$ spilling over into marker $j$, and the denominator is the marginal distribution. Putting all this together and applying Bayes rule leads to the posterior probability.
...
We propose a two step procedure for estimating spillover probability. In step 1, we initialize all marker distributions $P(Y_{ij} = y \mid Z_{ij} = k)$ and the mixture probabilities $\pi_{kj}$. We update the mixture component of the target marker $P(Y_{ij} = y \mid Z_{ij} = j)$ and the mixture probabilities using the EM algorithm [@dempster1977maximum]. In step 2, we fit a logistic function to the noisy probability estimates from step 2 using logistic regression. We use the smooth probability estimates to assign counts to spillover or signal.

Step 1 is standard EM algorithm with some application specific modifications: components come from two different data sources (beads experiment and real cells). We estimate the non-target mixture components for $P(Y_{ij} = y \mid Z_{ij} = k)$ with $j \ne k$ from beads experimental data. We estimate the target mixture component $P(Y_{ij} = y \mid Z_{ij} = j)$ from experiments with real cells, which we denote by the conditioning on the target marker $j$. Furthermore, we only update the target mixture component and the mixture probabilities during the E and M steps until convergence, while we keep the non-target mixture components fixed as estimated from the beads experiment.

### Step 1: EM Algorithm

We initialize the probability mass function using the nonparametric estimate,
$$
P^{(0)}(Y_{ij} =y \mid Z_{ij} = k) = \frac{\sum_{i = 1}^n \mathbf{1}(y_{ij} = y)}{\sum_{j = 1}^K \sum_{i = 1}^n \mathbf{1}(y_{ij} = y)},
$$
where $\mathbf{1}(\cdot)$ is the indicator function, $n$ is the total number of cells, and $K$ is the total number of spillover markers. We set the mixture probability vector to the discrete uniform, 
$$
...
* E step: We calculate the probability of a count $Y_i = y_{ij}$ spilling over from $j$ into $k$ at the current estimate $t$ of the marker distributions and mixture probability vector:
$$
P(Z_{ij} = k \mid Y_i = y_{ij}) = \frac{\pi_{kj} \, P^{(t)}(Y_{ij} = y \mid Z_{ij} = k)}{\sum_{k' = 1}^K \pi_{k'j} P^{(t)}(Y_{ij} = y \mid Z_{ij} = k')}.
$$
* M step: We update, $(t+1)$, the mixture probability vector, 
$$
\pi_k^{(t+1)} = \frac{1}{I} \sum_{i = 1}^{I} P(Z_{ij} = k \mid Y_i = y_{ij}),
$$ 
...
$$
P^{(t+1)}(Y_{ij} =y \mid Z_{ij} = k) = \frac{\sum_{i = 1}^n \mathbf{1}(y_{ij}^{(t+1)} = y)}{\sum_{j = 1}^K \sum_{i = 1}^n \mathbf{1}(y_{ij}^{(t+1)} = y)}.
$$

Here is a numerical example to illustrate the M step:
$$
y_j^{(t)} =
\begin{bmatrix}
3 \\ 5 \\ 17 \\ 3 \\ 17 \\ 2
...
y_j^{(t+1)} =
\begin{bmatrix}
\times \\ 5 \\ 17 \\ \times \\ 17 \\ \times
\end{bmatrix}.
$$
The vector of observations for the target marker $j$ has six cells. At iteration $t$, all six cells are assigned to the target marker. At the next iteration $t+1$, cell 1, 4, and 6 are assigned to the target marker. We denote counts assigned as non-target counts by $\times$. We estimate the target distribution at iteration $t$,
$$
\begin{aligned}
P^{(t)}(Y_{ij} = 2 \mid Z_{ij} = j)  & =  1/6 \\
P^{(t)}(Y_{ij} = 3 \mid Z_{ij} = j)  & =  2/6 \\
P^{(t)}(Y_{ij} = 5 \mid Z_{ij} = j)  & =  1/6 \\
P^{(t)}(Y_{ij} = 17 \mid Z_{ij} = j) & =  2/6,
\end{aligned}
$$
and $t+1$,
$$
\begin{aligned}
P^{(t+1)}(Y_{ij} = 5 \mid Z_{ij} = j)  & =  1/3 \\
P^{(t+1)}(Y_{ij} = 17 \mid Z_{ij} = j) & =  2/3.
\end{aligned}
$$

The M step relies on our assumption [(A1)](#assumption1) because we only update the distribution of the target marker. The E step relies on our assumption [(A2)](#assumption2) because we use the logit function, which is monotonic decreasing function. After $T$ iterations, the final output is the spillover probability curve with estimates at discrete points in the support of $Y_i$, 
$$
\hat{P}(\text{spillover} | Y_{ij} = y) = 1 - P^{(T)}(Z_{ij} = j \mid Y_{ij} = y).
$$

In our experiments, setting $T = 10$ was enough to reach convergence to reach convergence with many different initial values. \textcolor{red}{TODO:} [report something quantitative]

### Step 2: Logistic Regression

This step is a post-processing step of $\hat{P}(\text{spillover} \mid Y_i = y)$. Until now, the procedure is fully nonparametric. We apply no regularization to $y$'s that are close to one another. As a result, the spillover probability estimates are noisy and we regularize them using our assumption [(A3)](#assumption3).
...
Figure \@ref(fig:simulated-experiments) has four panels, one panel per experimental setup. The first row tests our assumptions [(A1)](#assumption1) and [(A2)](#assumption2). The second row tests assumption [(A3)](#assumption3) with two different experiments. For all experiments, we model counts using a Poisson distribution with parameter $\lambda$. We simulate 10,000 real cells with $\lambda = 100$, 1,000 beads with $\lambda = 70$, and spillover probability is $0.1$. We simulate bimodal distributions of bead and real cells using a mixture probability of $0.5$. Beads are an independent copy of the true spillover. The other parameters and statistical dependencies are specific to each experiment. The details of the generative models are in Appendix \@ref(generative-models). We quantify uncertainty with grey vertical lines representing the standard error computed over five replications of the same experiment.
...
\textcolor{red}{TODO:} [Resolve comment by Sebastian:

Unsure about this experiment:
Z_h is just like any other Z_j→i, isn't it?
All Z_j→i are unobserved.

Is the assumption that Y_i is indeed generated as per the mixture model you posit and there is no unobserved confounder that enters Y?
But what would you understand as a confounder here?
Also, does increasing beyond 0.5 make sense? In this case, we could say Z_h is actually Z.

One thing I can see is that your set-up violates "Y follows mixture models of underlying unobserved Z for which we have beads experiments to estimate P(Z_j to i)", but in what sense is this a confounder?
It seems to me that it's another mixture component that can generate Y for which we have no way of getting a meaningful estimate of the Poisson rate and for which we don't even model a mixture component, so it's model-misspecification where we model less components than there are.
What do you think?

If you set the rate of Z_h to 90, then wouldn't there simply be no way for us to realise even that we are observing sometimes Z and sometimes Zh, but would remove counts lower than 70 and attribute them to Z'?

So, perhaps the violation is: "undercomplete mixture model, there are more mixture components than observed and than can spillover distributions be estimated for in a beads experiment"

In this sense it seems A.3 may be somewhat analogous/related]
...
I think the data generating process can equivalently be rewritten following the standard model we posit in the main text, where there are 2 spillover and 1 true Z that mix, while the trick here seems to be that the premixing and subsuming Z1 and Z2 as one spillover-component somewhat "hides the separate identities of Z1 and Z2" – which is closely related to A.2.]
