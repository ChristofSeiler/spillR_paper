...
We choose three different experiments to test how robust `spillR` is when one of our three assumptions are violated. We explore a wide range of possible parameter settings. To visualize the massive amount of experiments, we summarize the full distributions with the mean of the observed, true, and `spillR` compensated distributions.

Figure \@ref(fig:simulated-experiments-plot) has three columns, each representing one experimental setup. The first column tests our assumptions [(A1)](#assumption1), the second [(A2)](#assumption2), and the third [(A3)](#assumption3). For all three experiments, we model counts using a Poisson distribution with parameter $\lambda$. We simulate 10,000 real cells with $\lambda = 100$, 1,000 beads with $\lambda = 70$, and spillover probability of $0.1$. Beads are an independent copy of the true spillover. The other parameters and statistical dependencies are specific to each experiment. The details of the generative models are in Appendix \@ref(generative-models). We quantify uncertainty with grey vertical lines representing the standard error computed over five replications of the same experiment. Figure \@ref(fig:simulated-experiments-plot) has four rows. The first row represents the summary of the means for each experimental setup as a function of their respective parameter $\tau$. This parameter has a difference meaning in each setup. 

<!--
### Bead Shift (A1)
-->

For [(A1)](#assumption1), $\tau$ is the shift of the measured beads spillover away from the true bead spillover. The solid curve is the measured data and the dashed curves is the true but unmeasured data. In our experiments, we test a wide range of bead shifts from $\tau = -30$ to no shift at $\tau = 0$. We observe this shift in the density plots in row two to four. At $\tau = -30$, the true spillover distribution is well separated from the actual observed spillover. Such low-quality beads cause both the observed and compensated mean to be below the true mean. As the the beads quality improve, the compensated signal move closer to the true mean, and at around $\tau = -13$ intersects with the true mean. At $\tau = 0$, we overestimate the mean by one.
...
\textcolor{red}{TODO: "The true unobserved distribution is close to the distribution of the spillR-corrected counts" etc – this also highlights "we do not correct all counts by shrinking them, but we remove some counts, in the hope that the distribution of the remaining counts that we keep is close to the true distribution" This seems to be a conceptual difference between Chevralier and your approach}
