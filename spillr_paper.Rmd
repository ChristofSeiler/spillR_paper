...
\textcolor{red}{
Note on the terminology: In mass cytometry, counts are called dual counts as they are calculated from real pulse ion counts for lower values and converted from intensity values for higher values using the dual count coefficient. Some authors refer to dual counts as signal intensity. We will call them counts in this paper to illustrate the fact that we rely heavily on the fact that they are non-negative integers as opposed to possibly real valued intensities. 
}
...
$$ 
and evaluate the $k$th mixture component using \textcolor{red}{its PMF after smoothing and renormalizing, $\widehat{P}(Y_i = y \mid Z_i = k)$.} We smooth the probability mass function (PMF), $\widehat{P}(Y_i = y \mid Z_i = k)$, 
\textcolor{red}{using kernel density estimation implemented in R function \texttt{density} with the default option for selecting the bandwith of the Gaussian kernel.} The procedure is not sensitive to the choice of the initial mixture probability vector. Other settings are possible, e.g., setting all probabilities to the same value.
...
\textcolor{red}{
We compare our mixture method to a fast baseline method that considers only the bead distributions. We estimate the bead PMF of each bead $k$ with the smoother function \texttt{density} in R, $\widehat{P}(Y_i | Z_i = k)$. We normalize the PMF at a specific count value $Y_i = y$ to one and calculate the spillover probability as, 
$$
...
\textcolor{red}{
The average computational time with 100 replications on an Apple M1 with 8 cores and 16 GB of RAM are $10.6$ seconds for \texttt{spillR}, $0.43$ seconds for \texttt{CATALYST}, and $0.45$ seconds for \texttt{spillR} (fast). The computational costs scale linearly in the number of cells. This allows for processing of large scale datasets.
}
...
\textcolor{red}{
We compare \texttt{spillR} and \texttt{CATALYST} on semi-simulated data. The goal is to elucidate the main differences between \texttt{spillR} and \texttt{CATALYST}, and to evaluate the performance of \texttt{spillR} on more than one spillover marker. We create semi-simulated datasets by overwriting the beads distribution for CD3 (Yb173Di). We take the first mode of the distribution of CD3 (Yb173Di) in real cells by filtering counts that range from 10 to 300. We shift these filtered counts by three different values: no shift is $0$, subtracting $0.47$ on the transformed scale, and subtracting $0.94$ on the transformed scale. We further sample without replacement from this new bead distribution to keep the same number of beads as in the original dataset. Figure \ref{fig:semi-simulated-plot} shows the three different datasets. In panel A, all bead distributions are equal to the original dataset in Figure \ref{fig:method-example} except Yb173Di is now perfectly aligned with the first mode of the distribution of real cell (red curve in B). In panel B and C, we shift the bead distribution of Yb173Di by $0.47$ and $0.94$. All three methods correctly compensate the spillover mode when no shift is present. \texttt{CATALYST} and \texttt{spillR} (fast) compensates more aggressively in the medium shift cases, while \texttt{spillR} is more moderate and compensate only the left hand tail of the spillover mode. In the largest shift of $0.94$, the three methods behave differently. \texttt{CATALYST} shrinks counts towards zero, shifting the entire spillover towards zero, \texttt{spillR} just compensates lightly on the left hand tail, and \texttt{spillR} (fast) compensate aggressively leaving only a small piece on the right hand tail.
}
