...
Step 1 is a standard EM algorithm with some application specific modifications: components come from two different data sources (beads experiment and real cells). We estimate the non-target mixture components $Y_{ij} \mid Z_{ij} = k$ with $j \ne k$ from beads experimental data. We estimate the target mixture component $Y_{ij} \mid Z_{ij} = j$ from experiments with real cells, which we denote by the conditioning on the target marker $j$. Furthermore, we only update the target mixture component and the mixture probabilities during the E and M steps until convergence, while we keep the non-target mixture components fixed as estimated from the beads experiment.

### Step 1: EM Algorithm

We refer to Appendix \@ref(generative-models) for a mathematical description of our EM algorithm. Here we illustrate the procedure with an numerical example. 

The E step: \textcolor{red}{TODO}
...
In our experiments, setting $T = 10$ was enough to reach convergence with many different initial values.
...
$$
P^{(0)}(Y_{ij} =y \mid Z_{ij} = k) = \frac{\sum_{i = 1}^n \mathbf{1}(y_{ij}^{(0)} = y)}{\sum_{j = 1}^K \sum_{i = 1}^n \mathbf{1}(y_{ij}^{(0)} = y)},
$$
where $y_{ij}^{(0)}$ is the element at the $i$th row and $j$th column in observed count matrix $\mathbf{Y} = (y_{ij})$ at iteration $t = 0$, $\mathbf{1}(\cdot)$ is the indicator function, $n$ is the total number of cells, and $K$ is the total number of spillover markers. We set the mixture probability vector to the discrete uniform, 
$$
...
$$
P(Z_{ij} = k \mid Y_i = y_{ij}^{(0)}) = \frac{\pi_{kj} \, P^{(t)}(Y_{ij} = y_{ij}^{(t)} \mid Z_{ij} = k)}{\sum_{k' = 1}^K \pi_{k'j} P^{(t)}(Y_{ij} = y_{ij}^{(0)} \mid Z_{ij} = k')}.
$$
