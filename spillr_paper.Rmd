...
\textcolor{red}{In all six panels (A--F), we observe that \texttt{spillR} compensates most strongly in the low counts, whereas \texttt{CATALYST} compensates strongly in the middle range. In the second column, where we expect spillover to be less pronounced, \texttt{CATALYST} compensates strongly in the first row, but little in rows two and three. \texttt{spillR} does the opposite: it compensates little in the first row, and more in rows two and three. \texttt{spillR-naive} compensates strongly in all instances. A characteristic pattern can be seen in panel C, CD3 (Yb173Di) against HLA-ABC (Yb172Di). \texttt{CATALYST}  compensates strongly in the middle range and removes the spherical pattern that shows correlation between the two markers. \texttt{spillR} preserves this correlation structure and only compensates the lower counts of CD3 (Yb173Di). This highlights a key difference between \texttt{spillR} and \texttt{CATALYST}: \texttt{spillR} identifies counts that may arise from spillover and replaces them with a user-specified value (e.g. 0, \texttt{NA}, or -1), whereas \texttt{CATALYST} shrinks counts across the entire range to compensate for spillover. \texttt{spillR-naive} compensates most aggressively, assigning a wide range of counts outside the target marker bead distribution to spillover.}
...
The original experiment shown in Figure \ref{fig:spillr-vignette} is designed to produce little spillover in the settings shown in the second column due to the metals used on the horizontal and vertical axes being very different.
Nonetheless, we find that \texttt{CATALYST} compensates substantially in panel B, and \texttt{spillR} compensates substantially in panels D and F.
Our diagnostic plots show overlap between the distribution of at least one spillover marker (as identified by the spillover matrix in \texttt{CATALYST}) on beads and the first mode real cell distribution.
Thus, the compensation by \texttt{spillR} is consistent with assumption (A1). This can also be seen in the results of \texttt{spillR-naive}, which considers all low counts as spillover, which happens if there is only spillover and no target marker signal for these counts in the bead experiment.
These results results indicate that the performance of both \texttt{spillR} and \texttt{CATALYST} rests on the plausibility of their respective assumptions in the individual case. Since \texttt{spillR} relies on a different set of assumptions, it may offer a complementary solution. Note that in principle it is also possible to combine the methods, for example by using \texttt{spillR} to identify spillover, and \texttt{CATALYST} to compensate for it.
}
