...
$$
P^{(0)}(Y_i = y \mid Z_i = 1) = \frac{\sum_{i = 1}^{n_y^{(0)}} \mathbf{1}\left(y_i^{(0)} = y\right)}{\sum_{i = 1}^{n_y^{(0)}} y_i^{(0)}}.
$$
The superscripts denote quantities that be updated throughout the EM iterations. Note, the count $y_i$ has a superscript because we update the data vector during the M step by masking elements that are due to spillover. We update---going from current estimate $t$ to the next $t+1$---the target distribution and the mixture probability vector using the hard-assignment EM algorithm:
...
$$
p^{(t)} = \pi_1^{(t)} \, P^{(t)}(Y_i = y^{(t)}_i \mid Z_i = 1) + \sum_{k = 2}^{K+1} \pi_{k}^{(t)} \, P(Y_i = y_i^{(t)} \mid Z_i = k).
$$
Note that the spillover marker distributions $P(Y_i = y_i^{(t)} \mid Z_i = k)$ are not updated.
* M step: We update the mixture probability vector to the next estimate $t+1$, 
$$
\pi_k^{(t+1)} = 
\begin{cases}
\frac{1}{n_y^{(t)}} \sum_{i = 1}^{n_y^{(t)}} P^{(t)}\left(Z_i = k \mid Y_i = y_i^{(t)}\right), & k = 1, \\
\frac{1}{n_x} \sum_{i = 1}^{n_x} P^{(t)}\left(Z_i = k \mid Y_i = y_i^{(t)}\right), & k > 1,
\end{cases}
$$ 
assign counts to the marker with the largest posterior probability, mask counts that belong to the spillover components, and re-estimate the distribution for the target marker with the newly assigned counts,
$$
P^{(t+1)}(Y_i = y \mid Z_{ij} = 1) = \frac{\sum_{i = 1}^{n_y^{(t+1)}} \mathbf{1}\left(y_i^{(t+1)} = y\right)}{\sum_{i = 1}^{n_y^{(t+1)}} y_i^{(t+1)}}.
$$
...
We rely on assumption [(A1)](#assumption1) to justify to only update the distribution of the target marker. We rely on assumption [(A2)](#assumption2) to justify calculating the spillover probability from the mixture model. We refer to Appendix \@ref(em-algorithm-example) for a numerical example of our EM algorithm. 
...
$$
\log\left(\frac{P(\text{spillover} \mid Y_i = y)}{1-P(\text{spillover} \mid Y_i = y)}\right) = \beta_0 + \beta_1 y,
$$
...
tb_summary <- tb_experiment %>% 
    group_by(title, tau) %>% 
    summarize(
      observed = mean(mean_obsv),
      truth = mean(mean_truth),
...
p_overview <- pivot_longer(tb_summary, -c(title, tau), 
  ...
  mutate(mean = factor(mean, levels = c("observed", "truth", "spillR"))) %>%
...
```{r spillr-vignette, fig.height = 6, fig.width = 10, out.width = "100%", fig.align = "center", echo = FALSE, warning = FALSE, message = FALSE, fig.cap="Comparison of compensation methods and uncorrected counts on real data. Arcsinh transformed counts with cofactor of 5."}
 # constants
bc_key <- c(139, 141:156, 158:176)
...
Here we illustrate the procedure with an numerical example with one target and one spillover marker. We have to data vectors $\mathbf{y}$ and $\mathbf{X}$,
$$
\mathbf{y}^{(0)} = \left(y_i^{(0)}\right) = 
\begin{bmatrix}
3  \\ 
5  \\ 
17 \\ 
...
\end{bmatrix}.
$$
The first vector represents the counts for the target marker measured in the experiment with real cells, and the second vector are counts from the beads experiment. In practice, the second vector is usually a matrix with multiple columns representing multiple spillover markers.
...
$$
\begin{aligned}
& P^{(0)}(Y_i = 2 \mid Z_i = 1)  =  1/6 \qquad & P^{(0)}(Y_i = 2 \mid Z_i = 2) = 1/3 \\
& P^{(0)}(Y_i = 3 \mid Z_i = 1)  =  2/6 \qquad & P^{(0)}(Y_i = 3 \mid Z_i = 2) = 2/3 \\
...
$$
\begin{aligned}
& P^{(1)}(Z_i = 1 \mid Y_i = 2)  = \frac{1/2 \cdot 1/6}{1/2 \cdot 1/6 + 1/2 \cdot 1/3} = 1/3 \qquad & P^{(1)}Z_i = 1 \mid Y_i = 2) = 1 - 1/3 = 2/3 \\
& P^{(1)}(Z_i = 1 \mid Y_i = 3)  = \frac{1/2 \cdot 2/6}{1/2 \cdot 2/6 + 1/2 \cdot 2/3} = 1/3 \qquad & P^{(1)}(Z_i = 1 \mid Y_i = 3) = 1 - 1/3 = 2/3 \\
...
$$
\pi_1^{(1)} = \frac{1/3 + 1/3 + 1 + 1}{4} = 2/3 \qquad \pi_2^{(1)} = \frac{2/3 + 2/3 + 0 + 0}{4} = 1/3,
$$
re-assign counts to the marker with largest posterior probability,
$$
\mathbf{y}^{(1)} = \left(y_i^{(1)}\right) = 
\begin{bmatrix}
\times \\ 
5      \\ 
17     \\ 
...
$$
\begin{aligned}
& P^{(1)}(Y_i = 2 \mid Z_i = 1)  =  0 \qquad & \widehat{P}(\text{spillover} \mid Y_i = 2) = 1 \\
& P^{(1)}(Y_i = 3 \mid Z_i = 1)  =  0       & \widehat{P}(\text{spillover} \mid Y_i = 3) = 1 \\
& P^{(1)}(Y_i = 5 \mid Z_i = 1)  =  1/3     & \widehat{P}(\text{spillover} \mid Y_i = 5) = 2/3 \\
& P^{(1)}(Y_i = 17 \mid Z_i = 1) =  2/3     & \widehat{P}(\text{spillover} \mid Y_i = 17) = 1/3.
\end{aligned}
$$
...
\end{aligned}
$$
The generative model for beads is an independent copy of $Y \mid Z > 0$.
...
Z                & \sim \text{Bernoulli}(0.1) & \qquad \text{(pick spillover)} \\
(Y \mid Z = 0)   & \sim \text{Poisson}(100)   & \qquad \text{(true signal)} \\
M                & \sim \text{Bernoulli}(\tau) & \qquad \text{(pick mode with probability } \tau \text{)} \\
(S \mid M = 0)   & \sim \text{Poisson}(70)    & \qquad \text{(spillover)} \\
(S \mid M = 1)   & \sim \text{Poisson}(130)  & \qquad \text{(high count spillover)} \\
