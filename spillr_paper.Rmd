...
$$ 
Now, we update these initialize values using the E and M step.
...
\end{bmatrix}.
$$
The crosses indicate what elements are re-assigned to the other component based on the posterior probability computed in the E step. Finally, re-estimate the distribution for the target marker and calculate the spillover probability estimate, 
$$
\begin{aligned}
...
( Y \mid Z = 1 )   & = S                 & \qquad \text{(spillover)}.
\end{aligned}
$$
...
(Y \mid Z = 0)   & = T            & \qquad \text{(true signal)} \\
(Y \mid Z = 1)  & \sim \text{Poisson}(70)    & \qquad \text{(spillover)}
\end{aligned}
$$
The generative model for beads is an independent copy of $Y \mid Z = 1$.
...
```{r exemplary-cases, fig.height=6, fig.width=9, out.width="100%", fig.align="center", fig.cap="Left panels: Frequency polygons before and after spillover compensation. Right panels: Density plot of spillover markers. The black dashed curve in the plot on the right represents the spillover probability estimated by our method.", echo=FALSE, warning=FALSE, message=FALSE}
