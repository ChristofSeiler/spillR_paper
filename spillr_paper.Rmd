...
$$
P^{(t+1)}(Z_{i \to i} = y) = \frac{n_y^{(t+1)}}{\sum_{j = 1}^J n_y^{(t+1)}}.
$$

We estimate $P(Z_{j \to i} = y)$ during the first iteration. We keep $P(Z_{j \to i} = y)$ fixed after initialization for bead distribution $j \ne i$ [(A1)](#assumption1). We keep updating $P(Z_{i \to i} = y)$ at every iteration on the newly identified set of cells that belong to marker $i$. The E-step relies on our assumption [(A2)](#assumption2).

The final output of this step is the spillover probability curve with estimates at discrete points in the support of $Y_i$, $\hat{P}(\text{spillover} | Y_i = y)$.

### Step 2: Logistic Regression

This step is a post-processing step of $\hat{P}(\text{spillover} | Y_i = y)$. Until now, the procedure is fully nonparametric. We apply no regularization to $y$'s that are close to one another. As a result, the spillover probability estimates are noisy and we need to regularize them using our assumption [(A3)](#assumption3).
