...
$$
P(Y_{ij} = y) = \sum_{k = 1}^K \pi_k \, P(Y_{ij} = y \mid Z_i = k).
$$
The probability of spillover of $k$ into itself corresponds to the proportion of true signal in the observed counts. The total of all the spillover probabilities equals to one, $\sum_k \pi_k = 1$. Conceptually, we divide the probabilities into true signal probability, $\pi_j$, and the probability of spillover, that is, the total probability of all but the $j$th probability, $\sum_{k \ne i} \pi_k$. The total number of markers $J$ in mass cytometry panels is between 30 and 40 [@bendall2011single]. Typically, only a small subset $K$ of three to four markers spill over into the target marker [@catalyst].

Experimentally, we only measure a sample from the distribution $Y_{ij}$. The probabilities $\pi_k$ and true distributions $P(Y_{ij} = y \mid Z_i = k)$ are unobserved, and we need to estimate them from data. In many applications, the mixture components, $P(Y_{ij} = y \mid Z_i = 1), \dots, P(Y_{ij} = y \mid Z_i = K)$, are in a parametric family, for example, the negative binomial distribution. Here we propose to fit nonparametric mixture components. We make three assumptions that render the components and mixture probabilities identifiable:

* <div id='assumption1'>(A1) Spillover distributions are the same in bead and real experiments.</div>

The distribution of $Y_{ij} \mid Z_i = k$ that spills over into the true marker $j$ is the same in beads and real cells. This assumption allows us to learn the spillover distributions of $Y_{i1} \mid Z_i = k, \dots, Y_{iJ} \mid Z_i = K$---except the true marker $Y_{ij} \mid Z_i = j$---from experiments with beads, and transfer them to the experiment with real cells. This assumption relies on high-quality bead experiments that measure spillover in the same range as the target biological experiment.

* <div id='assumption2'>(A2) Observed counts $Y_{ij}$ can only be due to any one of the $Y_{ij} = y \mid Z_i = k$.</div>
...
1 - P(Z_i = j \mid Y_{ij} = y) = 
1 - \frac{\pi_j \, P(Y_{ij} = y \mid Z_i = j)}{\sum_{j = 1}^K \pi_k \, P(Y_{ik} = y) \mid Z_i = k)}.
$$

To parse this calculation, recall that in mixture models the probability $\pi_k$ is the prior probability, $P(Y_{ij} = y \mid Z_i = k)$ is the conditional probability of obtaining count $y$ given the mixture component for marker $k$ spilling over into marker $j$, and the denominator is the marginal distribution. Putting all this together and applying Bayes rule leads to the posterior probability.
...
We propose a two step procedure for estimating spillover probability. In step 1, we initialize all marker distributions $P(Y_{ij} = y \mid Z_i = k)$ and the mixture probabilities $\pi_k$. We update the mixture component of the target marker $P(Y_{ij} = y \mid Z_i = j)$ and the mixture probabilities using the EM algorithm [@dempster1977maximum]. In step 2, we fit a logistic function to the noisy probability estimates from step 2 using logistic regression. We use the smooth probability estimates to assign counts to spillover or signal.

Step 1 is standard EM algorithm with some application specific modifications: components come from two different data sources (beads experiment and real cells). We estimate the non-target mixture components for $P(Y_{ij} = y \mid Z_i = k)$ with $j \ne k$ from beads experimental data. We estimate the target mixture component $P(Y_{ij} = y \mid Z_i = j)$ from experiments with real cells, which we denote by the conditioning on the target marker $j$. Furthermore, we only update the target mixture component and the mixture probabilities during the E and M steps until convergence, while we keep the non-target mixture components fixed as estimated from the beads experiment.

### Step 1: EM Algorithm

We initialize the probability mass function using the nonparametric estimate,
$$
P^{(0)}(Y_{ij} =y \mid Z_i = k) = \frac{\sum_{i = 1}^I I(y_{ij} = y)}{\sum_{j = 1}^J \sum_{i = 1}^I I(y_{ij} = y)},
$$
where $I(\cdot)$ is the indicator function, $I$ is the total number of cells, and $J$ is the total number of markers. We set the mixture probability vector to the discrete uniform, 
$$
\pi_{1 \to i}^{(0)}, \dots, \pi_{J \to i}^{(0)} = 1/J.
$$ 
...
$$
P(Z_i = k \mid Y_i = y_{ij}) = \frac{\pi_k \, P^{(t)}(Y_{ij} = y \mid Z_i = k)}{\sum_{k' = 1}^K \pi_{k'} P^{(t)}(Y_{ij} = y \mid Z_i = k')}.
$$
* M step: We update, $(t+1)$, the mixture probability vector, 
$$
\pi_k^{(t+1)} = \frac{1}{I} \sum_{i = 1}^{I} P(Z_i = k \mid Y_i = y_{ij}),
$$ 
...
$$
P^{(t+1)}(Y_{ij} =y \mid Z_i = k) = \frac{\sum_{i = 1}^I I(y_{ij}^{(t+1)} = y)}{\sum_{j = 1}^J \sum_{i = 1}^I I(y_{ij}^{(t+1)} = y)}.
$$
...
$$
\begin{aligned}
P^{(t)}(Y_{\cdot j} = 2 \mid Z_i = j)  & =  1/6 \\
P^{(t)}(Y_{\cdot j} = 3 \mid Z_i = j)  & =  2/6 \\
P^{(t)}(Y_{\cdot j} = 5 \mid Z_i = j)  & =  1/6 \\
P^{(t)}(Y_{\cdot j} = 17 \mid Z_i = j) & =  2/6,
\end{aligned}
$$
and $t+1$,
$$
\begin{aligned}
P^{(t+1)}(Y_{\cdot j} = 5 \mid Z_i = j)  & =  1/3 \\
P^{(t+1)}(Y_{\cdot j} = 17 \mid Z_i = j) & =  2/3.
\end{aligned}
$$
...
$$
\hat{P}(\text{spillover} | Y_{ij} = y) = 1 - P^{(T)}(Z_i = j \mid Y_{ij} = y).
$$
