...
Y = bind_cols(target = target, spillover = spillover)
Y
```
...
w1 <- rep(1/n1, n1)
w2 <- rep(1/n2, n2)
F1 <- ewcdf(Y$target,    weights = w1)
F2 <- ewcdf(Y$spillover, weights = w2)
par(mfrow = c(1, 2))
...
P_ZY <- bind_cols(target = c(2, 3, 5, 17), P_ZY)
P_ZY
```
