...
$$
where $\mathbf{1}(\cdot)$ is the indicator function, $n_x$ is the total number of cells measured in the beads experiment, and $k$ is a specific spillover marker.
...
$$
P^{(t+1)}(Y_i = y \mid Z_i = 1) = \frac{\sum_{i = 1}^{n_y^{(t+1)}} \mathbf{1}\left(y_i^{(t+1)} = y\right)}{\sum_{i = 1}^{n_y^{(t+1)}} y_i^{(t+1)}}.
$$
...
Figure \@ref(fig:simulated-experiments-plot) has three columns, each representing one experimental setup. The first column tests our assumptions [(A1)](#assumption1), the second [(A2)](#assumption2), and the third [(A3)](#assumption3). For all three experiments, we model counts using a Poisson distribution with parameter $\lambda$. We simulate 10,000 real cells with $\lambda = 100$, 1,000 beads with $\lambda = 70$, and spillover probability of $0.1$. Beads are an independent copy of the true spillover. The other parameters and statistical dependencies are specific to each experiment. The details of the generative models are in Appendix \@ref(generative-models). We repeat each simulation five times and report averages over the five replications. Figure \@ref(fig:simulated-experiments-plot) has four rows. The first row represents the summary of the means for each experimental setup as a function of their respective parameter $\tau$. This parameter has a different meaning in each setup. 
...
In all six panels, we observed that `spillR` compensates more aggressively in the low counts. In panel CD3 (Yb173Di) against HLA-ABC (Yb172Di), `CATALYST` compensates more aggressively in the middle range. It removes the spherical pattern that shows correlation between the two markers. `spillR` preserves this correlation structure and only masks out the lower counts of CD3 (Yb173Di). This highlights a key difference between `spillR` and `CATALYST`. We do not correct all counts by shrinking them, but we remove some counts, in the idea that the distribution of the remaining counts that we keep are close to the true distribution. `CATALYST` follows another strategy by shrinking not just low counts but by shrinking across the entire range.
...
The experiment for [(A1)](#assumption1) shows that the mean count after `spillR` correction is closer to the true mean over a wide range of bead shifts. This indicates that our method is stable with low quality bead experiments. The experiment for [(A2)](#assumption2) shows that our method is robust to model misspecification. Moreover, misspecification should not be a major issue as we can add all channels if necessary. The increase in computational cost is relatively minor as our method scales linearly in the number of spillover markers. The experiment for [(A3)](#assumption3) shows that the mean count after correction is still closer to the true mean even with a clear violation of this assumption. This justifies our strong parametric assumption on the spillover probability even when the assumption is violated.

In our comparison to `CATALYST` on real data, we observe the effect of the two different correction strategies. `CATALYST` essentially shrinks counts towards zero by minimizing a non-negative least squares objective. It assumes that spillover is linear up to counts of 5,000. The applied shrinkage is the same for low counts (e.g., below 10) and high counts (e.g., more than 100). `spillR` does not require linearity of the spillover, but assumes that the distribution on the beads experiment carries over to the real cells experiment. If counts are not on the spillover range, then they are not correction no matter how low their values are. If counts are in the spillover range, then we aggressively mask them. But even with this aggressive masking, we do not remove correlations between markers. The marker correlation between HLA-ABC (Yb172Di) and CD3 (Yb173Di) illustrated this point. `CATALYST` removes the positive correlation, whereas `spillR` keeps the correlation for the higher counts. Compensation methods should try to remove spillover while keeping biological meaningful signal. If not, then downstream analysis that rely on these correlations can be biased.
