...
```{r method-example, fig.height=7, fig.width=7, out.width="75%", fig.align="center", fig.cap='A: Gaussian kernel density plot of target and spillover markers. B: The solid black curve represents the spillover probability estimated with our method. C: Frequency polygons of marker CD3 (Yb173Di) comparing no correction, our methods, and \\texttt{CATALYST}. Zero counts are not plotted. See Table \\ref{tab:method-example-summary} for the zero counts and means. Counts are arcsinh transformed with cofactor of five (\\protect\\hyperlink{ref-bendall2011single}{Bendall \\emph{et al.}, 2011}). The vertical dashed line helps to interpret the spillover correction when CD3 (Yb173Di) is $2.725$. This is an interesting point as it balances between spillover counts from Yb172Di measured in beads (panel A) and the counts measured on real cells (panel C). \\textcolor{red}{The spillover probability at $2.725$ is close to one. This means we will set most counts to \\texttt{NA}\'s at this point.} After correction the read curve (panel C) will be adjusted downwards to the green curve (panel C) with our method \\texttt{spillR}. \\textcolor{red}{Our baseline method \\texttt{spillR} (fast) in cyan (panel C) is similar to \\texttt{CATALYST} for low and medium counts, and similar to \\texttt{spillR} for high counts.}', echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE}
# constants
...
combo$correction <- factor(
  combo$correction, 
  levels = c("none", "spillR", "spillR (fast)", "CATALYST")
  )
...
Panel B has a solid black and gray curves representing our spillover probability estimates. \textcolor{red}{We can see that the probability of spillover is close to one. In that case, our correction step assigns most cells to spillover, and keeps only few at the current value.} We add a black dashed line to all plots at the position $2.725$ to illustrate this point. Low counts have spillover probability of one, which means that our procedure assigns them to spillover and masks them from the sample by setting them to `NA` values. Counts above four stem from spillover with probability zero (and from the actual target with probability one), which means that our procedure keeps them at their raw uncorrected value.
...
levels(zeros_means$correction)[3] <- "\\texttt{spillR} (fast)"
...
Panel C displays the distribution of our target marker, CD3 (Yb173Di), before and after spillover correction. Our compensation method, `spillR`, masks most markers in the low counts up to two, \textcolor{red}{between 50\% to 100\% in the medium counts range between two and four}, and keeps counts above four. Larger counts are not affected by the correction, as can be seen by the overlap of both curves. In contrast, `CATALYST` shifts large counts to the left, and shifts medium counts to low counts or zero counts. Zero counts (or equivalently `NA` counts for `spillR`) and mean counts are shown in Table \@ref(tab:method-example-summary). 
\textcolor{red}{
As we do not know the true mean in this case, the comparison of means here serves merely as an illustration of the main differences between \texttt{spillR} and \texttt{CATALYST} and not to identify the correct method. Our baseline method \texttt{spillR} (fast) is similar to \texttt{CATALYST} in low and medium range, but keeps higher counts unchanged.
}

\textcolor{red}{
Note on the terminology: In mass cytometry, counts are called dual counts as they are calculated from real pulse ion counts for lower values and converted from intensity values for higher values using the dual count coefficient. Some authors refer to dual counts as signal intensity. We will call them counts in this paper as a reminder that we rely heavily on the fact that they are non-negative integers as opposed to possibly real valued intensities. 
}
...
To perform the spillover compensation, we draw from a Bernoulli distribution with the spillover probability as parameter to decide whether or not to assign a given count to spillover. We consider spillover counts as having no clear biological interpretation and mask them from our dataset while keeping all other counts. By default, we choose to set spillover counts to `NA` instead of zero to avoid zero-inflated distributions, \textcolor{red}{but the user can specify to set the spillover to any other value.}
...
In the first experiment (panel A), we shift the measured beads spillover away from the true bead spillover to probe [(A1)](#assumption1). We test a wide range of bead shifts from no shift at $\tau = 0$ to \textcolor{red}{$\tau = -10$. At $\tau = -10$,} the measured spillover (the first mode of the yellow density) is shifted away from the actual spillover (the blue density). Such low-quality beads cause both the observed and compensated mean to be below the true mean. As the beads quality improves, the compensated signal moves closer to the true mean. As we increase $\tau$ the first mode of the yellow density moves towards the blue density. This represents high-quality bead experiments. In all cases, even for bad beads experiments, our compensation improves the means. Our compensation also increases means, as it should, in contrast to e.g. @catalyst.
...
combo$method <- factor(
  combo$method, 
  levels = c("uncorrected", "spillR", "spillR (fast)", "CATALYST")
  )
...
\textcolor{red}{
\texttt{spillR} (fast) compensates most aggressively, removing everything that is not in the support of the beads distributions.
}

\textcolor{red}{
The panels D and F show no compensation for \texttt{CATALYST}. For the same panels our \texttt{spillR} methods compensates quite strongly. Our diagnostic plots using function \texttt{plotDiagnostics} in \texttt{spillR} give some indication on why our method compensates that strongly. The design of the original experiment is such that we expect no spillover in the channels on the vertical axis. But our diagnostic plots show that the bead distributions from spillover marker---as identified by the spillover matrix in \texttt{CATALYST}---exhibit strong overlap between at least one spillover marker and the first mode in the distribution of real cells. Thus, our strong compensation is consistent with our assumption (A1). The behavior of \texttt{spillR} (fast) corroborate our findings by completely removing all counts in that area. This occurs when there is only spillover and no signal from the target marker.
} 
<!-- [(A1)](#assumption1) -->
...
mark <- benchmark(
  ...
  "spillR (fast)" = spillR::compCytof(sce, sce_bead, marker_to_barc, 
                                      fast = TRUE, n_cores = 8),
...
\textcolor{red}{
The average computational time with 100 replications on an Apple M1 with 8 cores and 16 GB of RAM is $10.6$ seconds for \texttt{spillR}, $0.43$ seconds for \texttt{CATALYST}, and $0.45$ seconds for \texttt{spillR} (fast). The computational costs scale linearly in the number of cells and number of spillover markers. This allows for processing of large scale datasets.
}
...
\textcolor{red}{
We compare \texttt{spillR} and \texttt{CATALYST} on semi-simulated data. The goal is to elucidate the main differences between \texttt{spillR} and \texttt{CATALYST}, and to evaluate the performance of \texttt{spillR} on more than one spillover marker. We create semi-simulated datasets by overwriting the beads distribution for CD3 (Yb173Di). We take the first mode of the distribution of CD3 (Yb173Di) in real cells by filtering counts that range from $1.44$ to $4.79$ on the transformed scale. We shift these filtered counts by three different values: no shift is $0$, subtracting $0.47$ on the transformed scale, and subtracting $0.94$ on the transformed scale. We further subsample without replacement from this new bead distribution to keep the same number of beads as in the original dataset. Figure \ref{fig:semi-simulated-plot} shows the three different datasets.
}

\textcolor{red}{
In panel A, all bead distributions are equal to the original dataset from Figure \ref{fig:method-example} except Yb173Di is now perfectly aligned with the first mode of the distribution of real cells (red curve in B). In panel B and C, we shift the bead distribution of Yb173Di by $0.47$ and $0.94$. All three methods correctly compensate the spillover mode when no shift is present. \texttt{CATALYST} and \texttt{spillR} (fast) compensates more aggressively in the medium shift cases, while \texttt{spillR} is more moderate and compensates only the left hand tail of the spillover mode. In the largest shift case of $0.94$, the three methods behave differently. \texttt{CATALYST} shrinks counts towards zero, shifting the entire spillover towards zero, \texttt{spillR} just compensates lightly on the left hand tail, and \texttt{spillR} (fast) compensates aggressively leaving only a small piece on the right hand tail.
}
...
\textcolor{red}{
Our baseline method, \texttt{spillR} (fast), illustrates the behavior of more aggressive compensation by considering only the bead distribution. If our baseline method compensate aggressively in a certain range, then it means that there are only non-target beads observed in that range.
}
