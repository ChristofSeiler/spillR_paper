...
$$
The first mixing probability is the proportion of true signal in the observed counts. The other $K$ mixing probabilities are the proportions of spillover. The total sum of mixing probabilities equals one, $\sum_k \pi_k = 1$. The total number of markers in mass cytometry panels is between 30 and 40 [@bendall2011single], but only a small subset of three to four markers spill over into the target marker [@catalyst]. So, typically $K = 1+3$ or $K = 1+4$.
...
We propose a three step procedure for estimating the spillover probability. In step 1, we estimate spillover marker distributions from beads experimental data. In step 2, we estimate the target mixture component and mixture probabilities from real cells experimental data. We refine these estimates using the EM algorithm [@dempster1977maximum]. In step 3, we fit a logistic function to the noisy probability estimates from step 2 using logistic regression. We use smooth probability estimates to assign counts to spillover or signal.

Denote the $n \times K$ count matrix as $\mathbf{Y} = (y_{i})$ with real cells in the first column and the beads in columns two and higher. To simplify mathematical notation but without loss of generality, we assume that the number of cells form the real and bead experiment are the same $n$. In practice, the number of cells from beads experiments is much smaller than from real experiments.

### Step 1: Estimate Non-Target Mixture Components From Beads Experimental Data {-}

We use the empirical cumulative distribution function (CDF) that puts mass $w_i = 1/n$ at each data point, 
$$
\widehat{F}_k(y) = \frac{\sum_{i = 1}^n w_i \, \mathbf{1} \left( y_{ik} \le y \right)}{\sum_{i = 1}^n w_i} \quad \text{with } w_i = 1/n,
$$
and estimate the conditional probability mass function by evaluating the empirical CDF,
$$
\widehat{P}^{(0)}(Y_i = y \mid Z_i = k) = \widehat{F}_k(y) - \widehat{F}_k(y-1) \quad \text{for all} \quad k > 1,
$$
where $\mathbf{1}(\cdot)$ is the indicator function, $n$ is the total number of cells measured in the beads experiment, and $k$ is a specific spillover marker. The $k$th column of $\mathbf{Y}$ contains marker counts for the $k$th spillover marker, which represents the empirical spillover distribution of marker $k$ into the target marker, that is, the marker in the first column of $\mathbf{Y}$.
...
$$
\hat{\pi}_{1}^{(0)}, \dots, \hat{\pi}_{K}^{(0)} = 1/K,
$$ 
estimate the target mixture component $Z_i = 1$ from the experimental data with real cells that puts mass $w_i = 1/n$ at each data point, and evaluate its empirical CDF to obtain conditional probabilities,
$$
\widehat{P}^{(0)}(Y_i = y \mid Z_i = 1) = \widehat{F}_1(y) - \widehat{F}_1(y-1).
$$
The superscripts denote quantities that will be updated throughout EM iterations. We update---going from current estimate $t$ to the next $t+1$---the target distribution and the mixture probability vector using the EM algorithm.
...
$$
\widehat{P}^{(t)}\left(Z_i = k \mid Y_i = y_i \right) = 
\frac
{ \hat{\pi}_k^{(t)} \, \widehat{P}^{(t)}(Y_i = y_i \mid Z_i = k)          }
{ \sum_{k = 1}^K \hat{\pi}_k^{(t)} \, \widehat{P}^{(t)}(Y_i = y_i \mid Z_i = k) },
$$
Note that we keep the spillover marker distributions at the initial value,
$$
\widehat{P}^{(t)}(Y_i = y_i \mid Z_i = k) = \widehat{P}^{(0)}(Y_i = y_i \mid Z_i = k) \quad \text{for all} \quad k > 1.
$$

* M step: We update the mixture probability vector to the next estimate $t+1$, 
$$
\hat{\pi}_k^{(t+1)} = 
\frac{1}{n} \sum_{i = 1}^n \widehat{P}^{(t)}\left(Z_i = k \mid Y_i = y_i \right),
$$
and re-estimate the distribution for the target marker with the new posterior probabilities as weights,
$$
\widehat{F}_1^{(t+1)}(y) = \frac{\sum_{i = 1}^n w_i \, \mathbf{1} \left( y_{ik} \le y \right)}{\sum_{i = 1}^n w_i} \quad \text{with } w_i = \widehat{P}^{(t)}\left(Z_i = k \mid Y_i = y_i \right).
$$
...
$$
\begin{aligned}
& \widehat{P}^{(0)}(Y_i = 2 \mid Z_i = 1)  =  1/6 \qquad & \widehat{P}(Y_i = 2 \mid Z_i = 2) = 2/3 \\
& \widehat{P}^{(0)}(Y_i = 3 \mid Z_i = 1)  =  2/6 \qquad & \widehat{P}(Y_i = 3 \mid Z_i = 2) = 1/3 \\
& \widehat{P}^{(0)}(Y_i = 5 \mid Z_i = 1)  =  1/6 \\
& \widehat{P}^{(0)}(Y_i = 17 \mid Z_i = 1) =  2/6.
\end{aligned}
$$
We initialize the mixture probabilities with the discrete uniform, 
$$
\hat{\pi}_1^{(0)} = 1/2 \qquad \hat{\pi}_2^{(0)} = 1/2.
$$ 
...
$$
\begin{aligned}
& \widehat{P}^{(1)}(Z_i = 1 \mid Y_i = 2)  = \frac{1/2 \cdot 1/6}{1/2 \cdot 1/6 + 1/2 \cdot 2/3} = 1/5 \qquad & \widehat{P}^{(1)}(Z_i = 2 \mid Y_i = 2) = 1 - 1/5 = 4/5 \\
& \widehat{P}^{(1)}(Z_i = 1 \mid Y_i = 3)  = \frac{1/2 \cdot 2/6}{1/2 \cdot 2/6 + 1/2 \cdot 1/3} = 1/2 \qquad & \widehat{P}^{(1)}(Z_i = 2 \mid Y_i = 3) = 1 - 1/2 = 1/2 \\
& \widehat{P}^{(1)}(Z_i = 1 \mid Y_i = 5)  = 1 \\
& \widehat{P}^{(1)}(Z_i = 1 \mid Y_i = 17) = 1.
\end{aligned}
$$
* M step: Update the mixing probability vector,
$$
\hat{\pi}_1^{(1)} = \frac{1/2 + 1 + 1 + 1/2 + 1 + 1/5}{6} = 7/10 \qquad \hat{\pi}_2^{(1)} = 1 - \hat{\pi}_1^{(1)} = 3/10,
$$
...
$$
\begin{aligned}
& \widehat{P}^{(1)}(Y_i = 2 \mid Z_i = 1)  =  0.0476 \qquad & \widehat{P}(Y_i = 2 \mid Z_i = 2) =  2/3 \\
& \widehat{P}^{(1)}(Y_i = 3 \mid Z_i = 1)  =  0.238 & \widehat{P}(Y_i = 3 \mid Z_i = 2) = 1/3 \\
& \widehat{P}^{(1)}(Y_i = 5 \mid Z_i = 1)  =  0.238 & \\
& \widehat{P}^{(1)}(Y_i = 17 \mid Z_i = 1) =  0.476 &
\end{aligned}
$$
