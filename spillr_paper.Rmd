...
# overwrite with simulated data
test <- assay(sce, "counts")
rn <- rownames(test)
test <- rpois(n = length(test), lambda = 5) |> 
  matrix(nrow = nrow(test), ncol = ncol(test))
rownames(test) <- rn
assay(sce, "counts") <- test
assay(sce, "exprs") <- tfm(test)

sce <- CATALYST::compCytof(sce, sm, method = "nnls", overwrite = FALSE)
