...
\textcolor{red}{Discussion point:
Chevrier in their \\
"Compensation Corrects for Spillover-Mediated Artifacts in Mass Cytometry"
section focus a lot on correlations and that those correlations were due to spillover
– do you see why/how they can argue that the correlations were due to spillover and not biologically meaningful? \\
I may be missing something, but I feel their argument is circular: assuming catalyst removes the right things, then observing a reduction in correlations must mean the correlations were due to spillover which catalyst removed. \\
We write it's good to preserve correlations, which is in stark contrast.
This seems like a contraposition to our claim that we may not have a reason to believe those correlations are due to spillover – do we address this or can we say something about this? Perhaps better if we do it rather than waiting for a reviewer to prompt us "but chevrier showed that those correlations are due to spillover, so preserving them is not a good thing after all"?}
...
This example highlights the main difference between our method and the one by @catalyst. After masking cell marker CD4 (ND145Di) counts that are likely due to spillover, the average of the remaining CD4 (ND145Di) counts will be larger than that of all CD4 (ND145Di) counts. So, in this example, our spillover correction method increases the average count. By contrast, the method by @catalyst will shrink all the counts towards zero. As a consequence, the average count will be lower after correction.
...
\textcolor{red}{Is the Yb172-Yb172 bead experiment also showing a peak in Yb172 comparable to the Yb172 peak observed in real data? \\
For the beads to be high-quality we would probably want the beads experiments to have similar peaks in all markers as compared to the multi-stained real-cells? \\
Otherwise, one may argue that the spilled-into blob around counts of 4 from Yb172 is not realistic as the true Yb172 signal and thus the spillover may be higher/lower in the real cells? \\
I think this relates to Chevrier establishing that spillover is linear – if true signal is higher, spillover is higher – so if there is two cell types in the real data, one with mean-6 CD3 and mean-6-Yb172 that spills into Yb173, and one with mean-10-Yb172 that spills into Yb173 with higher mean, we would correct both similarly from spillover from Yb172 into Yb173.\\
If we stick true to our assumption 1, the optimal bead experiments would probably be those, where the peaks in markers have the same distribution as the peaks in the real experiment, i.e. we may need beads with a Yb173 peak around 6, but perhaps also some with a peak at around 12 if there is real cells with Yb173 peaks at around 12, too – does this make sense? If so, should we discuss this very briefly in the last section? \\
(If I understand correctly, in cyto data one often "gatse out" some cells which have a marker in a certain range, so they would perhaps often not assume that there is only cells with one Yb173 peak but also others which may have another peak?)}
...
Panel B has a solid black and gray curves representing our spillover probability estimates. With the smoothing parameter $k = 11$, we can see that the probability of spillover goes up to around $0.5$. In that case, our correction step assigns around 50% of cells to spillover, and keeps the other 50% at the current value. Low counts have spillover probability of one, which means that our procedure assigns them to spillover and masks them from the sample by setting them to `NA` values. Counts above four stem from spillover with probability zero (and from the actual target with probability one), which means that our procedure keeps them at their raw uncorrected value. The estimated curve with smoothing parameter $k = 3$ is more irregular, suggesting that these fluctuation may be driven by noise. Users can control the smoothing parameter $k$ to choose the desired bias-variance tradeoff.
...
\textcolor{red}{Perhaps we can motivate this: \\
Perhaps this makes sense as one would not stain such that the same cell type would have markers that are close to each other? \\
I.e. if we stain some cells with Yb173, say to detect cell type A, then observing a cell with Yb173 at around 6 would probably tell us that this is a cell of type A and that the Yb173 value stems from the true distribution. \\
If we instead observed a Yb173 value of 3, we would perhaps attribute this to spillover and conclude that this was not a type A cell. instead, perhaps, it was a type B cell that we stained with Yb172 and that shows a Yb172 count of 14 (and also spilled a bit into Yb173)? \\
I am not sure I am making sense, but I think this assumption may actually be quite plausible: We would probably want to avoid staining the same cell with Yb172 and Yb173 as those spill into each other, so if we observe a Yb173 value, we only need to decide "is this a plausible value to have come from a cell that was stained with Yb173 or is it more likely spillover from a cell that was marked with Yb172"}
...
# --------- helper function ---------
plot_critical <- function(experiment, taus, simulated, reverse = F) {
...
p_a <- plot_critical(experiment, taus, simulated, reverse = T)
...
We choose three different experiments to test `spillR` against different bead and real cell distributions. We explore a wide range of possible parameter settings. Figure \@ref(fig:simulated-experiments-plot) has three panels, each representing one experimental setup. The first two panels test our assumptions [(A1)](#assumption1) and [(A2)](#assumption2). The third panel tests sensitivity of `spillR` to bimodal bead distributions. For all three experiments, we model counts using a Poisson distribution with parameter $\lambda$. We simulate 10,000 real cells with $\lambda = 100$, and 1,000 beads with $\lambda = 70$, and spillover probability of $0.1$. Beads are an independent copy of the true spillover. The other parameters and statistical dependencies are specific to each experiment. The details of the generative models are given in Appendix \@ref(generative-models). We repeat each simulation 20 times and report averages over the 20 replications. Each panel of Figure \@ref(fig:simulated-experiments-plot) has two rows of plots. The plot on the first row represents the summary of the means for each experimental setup as a function of their respective parameter $\tau$. This parameter has a different meaning in each setup. To visualize the different experiments, we summarize the full distributions with the true simulated signal mean (black), the uncorrected mean (orange), and the `spillR` corrected mean (green). Plots on the second row illustrate the simulated data distributions for three selected parameters $\tau$ picked from the experimental setup. The yellow density curve is the observed count $Y$. The black density curve is the target cell count. The blue density curve is the spillover distribution. The goal of the experiment is to estimate the mean of the black density as accurately as possible from the yellow density curve, which represent the data $Y$ that we would observe in practice. We simulate this data ourselves with the models in Appendix \@ref(generative-models).

<!--
### Bead Shift (A1)
-->

\textcolor{red}{"measured spillover" twice at tau = -30 the spillover in the simulated real cells has mean 40 instead of the spillover with mean 70 in the simulated bead observations \\
maybe it's a good place to re-iterate, that our compensation increases means, as it should, in contrast to e.g. chevrier; \\
also, in all cases, even for bad beads experiments, the compensation improves the means, I think that's noteworthy (similar to panel B below), the compensation does not fail, it just corrects less well if the beads are not well aligned with the real experiment}
...
In the second experiment (panel B), we add a hidden spillover marker to explore the robustness of our method with respect to our second assumption [(A2)](#assumption2). One way to think about this is that the hidden spillover is a form of model misspecification. Our mixture model is undercomplete, which means that there are more true mixture components than we observe in the beads experiment. If $\tau = 0$, then assumption [(A2)](#assumption2) is correct; otherwise assumption [(A2)](#assumption2) is violated. \textcolor{red}{(though, I am not actually sure this violates A2, see next comment)} At $\tau = 0.1$, the hidden spillover is as large as the measured spillover. The compensated means are closer to the true means across the test range. The observed mean diverges from the true mean with increasing $\tau$ due to the transfer of mass from the signal to the spillover distribution.
\textcolor{red}{I am not sure this is a violation of A2 as long as both $Z=2$ and $Z=3$ have the same lambda? \\
Doesn't this experiment then just amount to having more or less spillover, that is, different $pi_i$ probabilities? \\
I somehow think the hidden spillover component should have a different Poisson parameter, so that it's truly a different component - the beads experiment would be with $\lambda=70$, while the real cells could be the true component, the $\lambda=70$ component or the $\lambda=X$ component, while only modelling one spillover component, then A2 would be violated? although, strictly speaking, that would still not violate A2 as we could view the two components as one (since we do not assume that the hidden component follows a Poisson distribution); a violation of A2 would probably be adding the spillover to the real count in some fraction of the cases, i.e. \\
$Y | Z = 1 \sim (1-\tau)marker + \tau*spillover$ \\
$Y | Z = 2 \sim (1-\tau)spillover + \tau*marker$ \\
so for $\tau=0$ no violation of the assumption, but for $\tau=0.5$ maximal violation}
...
\textcolor{red}{uncorrected data is the data from the machine with some inherent preprocessing and may not be counts? \\
for spillr, we take this observed data, round it down to the closes integer (is this the common practice also used by CATALYST?) and then treat it as count data, do our spillR correction which just masks some counts but all values remain valid count integer data?}
...
\textcolor{red}{Not only bimodality, but also if the spillover is actually larger than the true signal}
...
\textcolor{red}{We can explicitly mention that correlations are used to identify co-regulated proteins/genes \\
"Characterization of newly identified clusters or signaling network inference often involves the systematic correlation analysis of markers at the single-cell level to identify co-regulated proteins or genes, and this approach can be strongly affected by channel interference."}
