...
* Initialization step: We set the mixture probability vector to the discrete uniform, 
$$
\hat{\pi}_{1}, \dots, \hat{\pi}_{K} = 1/K,
$$ 
...
* M step: We estimate the new mixture probability vector based on posterior probabilities, 
$$
\hat{\pi}_k^{(\text{new})} = 
...
$$
\widehat{P}^{(\text{new})}(Y_i = y \mid Z_i = 1) = \widehat{F}_1(y) - \widehat{F}_1(y-1) \text{ with } w_i = \widehat{P} \left(Z_i = 1 \mid Y_i = y \right).
$$
