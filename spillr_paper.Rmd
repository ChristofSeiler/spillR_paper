...
\textcolor{red}{TODO:} [revised with notation from https://stephens999.github.io/fiveMinuteStats/intro_to_em.html]
...
The spillover probability is the probability that the counts are not due to the marker of interest but result of another marker spilling over into $Y_i$. The spillover probability is a function of $y$. Sufficiently large $y$ are not due to spillover and only carry the true signal. In contrast, smaller $y$ are more likely a result of another marker spilling over. Although it is biologically possible that spillover happens at higher counts, this assumption makes our statistical procedure more stable. We chose the logistic function as it is the standard option for logistic regression.

## Estimation of Spillover Probability

\textcolor{red}{TODO:} [clearly state what is done on beads, and what on real]

We propose a two step procedure for estimating spillover probability. In step 1, we initialize all marker distributions $P(Y_{ij} = y \mid Z_i = k)$ and the mixture probabilities $\pi_k$. We update the mixture component of the target marker $P(Y_{ij} = y \mid Z_i = j)$ and the mixture probabilities using the EM algorithm [@dempster1977maximum]. In step 2, we fit a logistic function to the noisy probability estimates from step 2 using logistic regression. We use smooth probability estimates to assign counts to spillover or signal.

Step 1 is the standard EM algorithm with some application-specific modifications: components come from two different data sources (beads experiment and real cells). We estimate the non-target mixture components for $P(Y_{ij} = y \mid Z_i = k)$ with $k \ne j$ from experimental data obtained on beads. We estimate the target mixture component $P(Y_{ij} = y \mid Z_i = j)$ from experiments with real cells, which we denote by the conditioning on the target marker $j$. During the E and M steps, we only update the target mixture component and the mixture probabilities, while we keep the non-target mixture components fixed at their estimates from the beads experiment.

### Step 1: EM Algorithm

\textcolor{red}{TODO:} Still working on it!
...
$$
P(Z_i = k \mid Y_{ij} = y) = \frac{\pi_k \, P^{(t)}(Y_{ij} = y \mid Z_i = k)}{\sum_{k' = 1}^K \pi_{k'} P^{(t)}(Y_{ij} = y \mid Z_i = k')}.
$$
* M-step: We update, $(t+1)$, the mixture probability vector, 
$$
\pi_k^{(t+1)} = \frac{1}{I} \sum_{i = 1}^{I} P(Z_i = k \mid Y_i = y_{ij}),
$$ 
re-assign counts to the marker with the largest posterior probability, and re-estimate the distribution for the target marker with the newly assigned counts,
$$
P^{(t+1)}(Y_{ij} =y \mid Z_i = k) = \frac{\sum_{i = 1}^I I(y_{ij}^{(t+1)} = y)}{\sum_{j = 1}^J \sum_{i = 1}^I I(y_{ij}^{(t+1)} = y)}.
$$
\textcolor{red}{TODO:} [add sketch going from $y_{ij}^{(t)}$ to $y_{ij}^{(t+1)}$]
...
In our experiments, setting $T = 10$ was enough to reach convergence with many different initial values. \textcolor{red}{TODO:} [report something quantitative]
