...
abstract: |
  In mass cytometry, channel interference can cause spillover and overcount abundances. @catalyst introduced an experimental and computational procedure to estimate and compensate for spillover. Their R package `CATALYST` implements this using non-negative least squares. They assume spillover can be approximated by a spillover matrix that encodes proportions. They estimate the spillover matrix from experiments with beads. We propose to skip the matrix estimation step and work directly with the full bead distributions. We develop a nonparametric finite mixture model, and use the mixture components to estimate the probability of spillover. We implement this in R package `spillR` using the EM algorithm and logistic regression. We test our procedure on synthetic and real data from `CATALYST`. We find that our method compensates more in the low counts. This property is favorable when high quality bead experiments are available.
---
...
In order to illustrate the main idea of our work, we can examine a simple example with only two markers (Figure \@ref(fig:intro-example)). The left panel depicts an experiment using beads tagged for CD36 (Nd144Di), with spillover occurring on the vertical axis. In the absence of spillover, ND145Di would have only values at zero. The right panel shows an experiment conducted with real cells, where we expect non-zero counts on both axis due to spillover in addition to the true biological counts. While the precise amount of spillover is unknown, it can be estimated using the bead experiment. We treat the spillover marker CD4 (Nd145Di) and the true---but unmeasured---marker CD36 (Nd144Di) as a mixture. Our statistical objective is to separate the true signal from the spillover in this mixture.

This paper presents a statistical approach for separating the true signal from spillover based on a mixture model (Section \@ref(methods)). To evaluate our methodology, we conducted experiments on both simulated and real data obtained from the `CATALYST` R package [@catalyst], utilizing our newly developed R package, `spillR` (Section \@ref(results)). After interpreting our results, we give a recommendation when to use `spillR` over `CATALYST` (Section \@ref(discussion)).
...
All the experiments and plots can be reproduced by compiling the R markdown file `spillR_paper.Rmd` on GitHub: 
