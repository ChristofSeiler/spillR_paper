...
$$ 
The procedure is not sensitive to the choice of the initial mixture probability vector. Other settings are possible but may be slower to converge, e.g., setting all probabilities to the same value. Then, we initialize the $k$th mixture component using \textcolor{red}{its probability mass function (PMF) after smoothing and renormalizing, $\widehat{P}(Y_i = y \mid Z_i = k)$.} We smooth the PMF, $\widehat{P}(Y_i = y \mid Z_i = k)$, \textcolor{red}{using kernel density estimation implemented in R function \texttt{density} with the default option for selecting the bandwith of the Gaussian kernel.}
...
\textcolor{red}{
We compare our mixture method to a fast baseline method that considers only the bead distributions. We estimate the bead PMF of each bead $k$ with the kernel density estimator \texttt{density} in R, $\widehat{P}(Y_i | Z_i = k)$. For all count values $y$ in the range of the bead counts, we separately normalize the PMF at each value $Y_i = y$ and calculate the spillover probability as, 
$$
