...
\end{bmatrix}.
$$
This matrix cannot be used for linear algebra because it is has different sized columns. Also, the order of rows can be permuted without loosing information. For example, counts on the first row are not measured on the same cell. The first column represents the counts for the target marker measured in the experiment with real cells, and the second column are counts from the beads experiment.
