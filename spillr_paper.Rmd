...
Panel C displays the distribution of our target marker, CD3 (Yb173Di) before and after spillover correction. \textcolor{red}{We observe few real counts (red) below a value of $2$, so although all methods perform strong compensation in this range, there is little visible change. Above a value of $2$ there is a clear distinction between the methods. \texttt{CATALYST}, like our baseline \texttt{spillR (naive)}, compensates nearly all counts forming the first peak of the raw counts (red) as spillover. By contrast, \texttt{spillR} compensates only where the relative frequency of the raw counts (red) match the density of spillover markers in the bead experiment shown in panel A. As a result, it does not compensate all of the counts forming the first peak of the red curve. While \texttt{CATALYST} shifts the distribution of large counts (around 6) slightly to the left, our methods leave them unaffected as the bead experiment shows no spillover in this region.}
