...
* E-step: We calculate the probability of a count $Y_i = y$ measured on cell $k$ actually belonging to marker $j$ from the current estimate of the marker distributions and mixture probability vector:
$$
P(\text{count on cell } k \text{ belongs to marker } j \, | \, Y_i = y) = \frac{\pi_{j \to i}^{(t)} \, P^{(t)}(Z_{j \to i} = y)}{\sum_{j' = 1}^J \pi_{j' \to i}^{(t)} \, P^{(t)}(Z_{j' \to i} = y)}.
$$
* M-step: First, we update the mixture probability vector, 
$$
\pi_{j \to i}^{(t+1)} = \frac{1}{N_j^{(t)}} \sum_{k = 1}^{N_j^{(t)}} P(\text{count on cell } k \text{ belongs to marker } j \, | \, Y_j = y),
$$
