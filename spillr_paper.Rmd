...
$$
P^{(0)}(Y_i = y \mid Z_i = 1) = \frac{1}{n_y^{(0)}} \sum_{i = 1}^{n_y^{(0)}} \mathbf{1}\left(y_i^{(0)} = y\right).
$$
...
$$
\pi_k^{(t+1)} = 
\frac{1}{n_y^{(t)}} \sum_{i = 1}^{n_y^{(t)}} P^{(t)}\left(Z_i = k \mid Y_i = y_i^{(t)}\right),
$$
...
$$
P^{(t+1)}(Y_i = y \mid Z_i = 1) = \frac{1}{n_y^{(t+1)}} \sum_{i = 1}^{n_y^{(t+1)}} \mathbf{1}\left(y_i^{(t+1)} = y\right).
$$
...
$$
and mask (indicated by an $\times$) elements in vector $\mathbf{y}$ whose posterior probability of belonging to marker 2 is larger than that of belonging to marker 1 (in case of ties, we keep elements assigned to marker 1),
$$
...
\end{bmatrix}.
$$
Finally, re-estimate the distribution for the target marker, keep the non-target marker at its initial value, 
$$
\begin{aligned}
& P^{(1)}(Y_i = 2 \mid Z_i = 1)  =  0 \qquad & P(Y_i = 2 \mid Z_i = 2)  =  2/3 \\
& P^{(1)}(Y_i = 3 \mid Z_i = 1)  =  2/5      & P(Y_i = 3 \mid Z_i = 2)  =  1/3 \\
& P^{(1)}(Y_i = 5 \mid Z_i = 1)  =  1/5      & \\
& P^{(1)}(Y_i = 17 \mid Z_i = 1) =  2/5      &
\end{aligned}
$$
and calculate the spillover probability estimate,
$$
\begin{aligned}
\widehat{P}(\text{spillover} \mid Y_i = 2)  & = 1 - \frac{7/10 \cdot 0}{7/10 \cdot 0 + 3/10 \cdot 2/3} = 1 \\
\widehat{P}(\text{spillover} \mid Y_i = 3)  & = 1 - \frac{7/10 \cdot 2/5}{7/10 \cdot 2/5 + 3/10 \cdot 1/3} = 0.74 \\
\widehat{P}(\text{spillover} \mid Y_i = 5)  & = 1 - \frac{7/10 \cdot 1/5}{7/10 \cdot 1/5 + 3/10 \cdot 0} = 0 \\
\widehat{P}(\text{spillover} \mid Y_i = 17) & = 1 - \frac{7/10 \cdot 2/5}{7/10 \cdot 2/5 + 3/10 \cdot 0} = 0.
\end{aligned}
$$
