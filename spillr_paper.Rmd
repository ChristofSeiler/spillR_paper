...
```{r intro-example, fig.height=3, fig.width=6, out.width="65%", fig.align="center", fig.cap="Two-dimensional histograms of cell counts. The blue shaded rectangle is an estimate of spillover from CD36 (Nd144Di) into CD4 (Nd145Di). Our method compensates for some counts that fall inside this rectangle. Counts are arcsinh transformed with cofactor of five (\\protect\\hyperlink{ref-bendall2011single}{Bendall \\emph{et al.}, 2011}).", echo=FALSE, warning=FALSE, message=FALSE}
# constants
...
```{r method-example, fig.height=5, fig.width=12, out.width="100%", fig.align="center", fig.cap='Left panel: Frequency polygons of marker CD3 (Yb173Di) before and after spillover compensation. Right panel: Gaussian kernel density plot of spillover markers. The solid black curve represents the spillover probability estimated with our method from the black points. Counts are arcsinh transformed with cofactor of five (\\protect\\hyperlink{ref-bendall2011single}{Bendall \\emph{et al.}, 2011}).', echo=FALSE, warning=FALSE, message=FALSE}
# constants
bc_key <- c(139, 141:156, 158:176)
...
run_spillr <- function(k) {
  sce_spillr <- compCytof(sce, sce_bead, marker_to_barc, overwrite = FALSE, 
                          runmed_k = k)
  p_pair <- plotDiagnostics(sce_spillr, "Yb173Di")
  p_pair <- lapply(p_pair, 
                   function(p) p + xlab("CD3 (Yb173Di)") + xlim(c(0, 7)))
  p_pair
}
p_pair_k3 <- run_spillr(3)
p_pair_k7 <- run_spillr(7)
# remove legend
#p_pair_k3 <- lapply(p_pair_k3, function(p) p + theme(legend.position="none"))
# combine everything
plot_grid(
  plot_grid(plotlist = p_pair_k3, nrow = 2, align = "v"),
  plot_grid(plotlist = p_pair_k7, nrow = 2, align = "v"),
  nrow = 1, labels = c("A", "B")
)
```

\textcolor{red}{Sebastian: 
2 ideas: \\
a) show another compensation method on the left panel? this could highlight how other methods shrink all values – the figure would then highlight "this may actually be a bit odd to shrink all values", one could change the legend to "Correction: none, spillR, other" \\
b) show mean values of the count distributions or indicate them on the x-axis (esp for left panel, as "increased mean" is a quite distinct feature of spillR compared to other compensation approaches)
– this will also help to highlight in the visual that the correction does actually do quite a bit (in the density plot alone, the 50% cut-down at around 2.5 is not that clear) \\
PS. could a wider/different kernel help highlight the density differences?}
...
The spillover probability, $P(\text{spillover} \mid Y_i = y)$, is the probability that the counts are not due to the marker of interest but the result of another marker spilling over into $Y_i$. The spillover probability is a function of $y$. Under (A3), smaller $y$ are more likely due to another marker spilling over, whereas sufficiently large $y$ are not due to spillover and only carry the true signal. Although it is biologically possible that spillover results in high counts, this may be considered unlikely and the assumption makes our statistical procedure more stable. For example, the bead experiment in Figure \@ref(fig:method-example), top panel, shows that the spillover occurs in lower counts. We model the spillover probabilities using logistic regression.
...
To refine our estimates, we iterate over the E and M-steps until estimates stabilize \textcolor{red}{Sebastian: What exact criterion used?}. The final output is the spillover probability curve with estimates at discrete points in the support of $Y_i$, 
$$
...
$$
The $\widetilde{P}(\text{spillover} \mid Y_i = y)$ is a smooth version of the noisy estimate from step 1, $\widehat{P}(\text{spillover} \mid Y_i = y)$. We obtain the last equation by solving the logistic regression model for the spillover probability and plugging in the MLE estimate, $\hat{\beta}_0$ and $\hat{\beta}_1$ for $\beta_0$ and $\beta_1$.
...
d_a2 <- expand.grid(
  ...
  replicate = 1:n_rep,
  title = "(A2) Model Misspecification"
  ) %>%
  rowwise() %>% 
...
plot_critical <- function(experiment, tau, simulated) {
  ...
  plot_grid(
    ggdraw() + 
      draw_label(experiment, fontface = "bold", x = 0, hjust = 0) + 
...
The color code of the two-dimensional histograms indicates the absolute number of cells that fall into one hexagon bin. The uncorrected and `spillR` corrected histograms can contain different absolute number of cells because of `spillR` rounding. The raw mass cytometry data is often not count data due to proprietary post-processing of the manufacturer of the mass cytometer. That is why we convert mass cytometry data to count data, we convert the raw values to the next lower integer. The uncorrected counts do not undergo this pre-processing step. 

# Discussion

\textcolor{red}{Sebastian: \\
adding channels as potential sources of spillover will change the initialisation quite a bit as we currently use 1/K for all; \\
perhaps that's another motivation to use a fixed proportion for the first-true-signal component and distribute the remainder evenly between all spilling-into-components? \\
while 1/2 for the true-signal component would be higher than what we currently initialise on, it may still be low compared to what people believe to be attributable to true signal vs spillover?
}
...
We thank EuroBioC2022 for awarding Marco Guazzini a travel award to present a preliminary version of `spillR` in Heidelberg. \textcolor{red}{Alexander, can you check with him if this is OK: We thank Antoine Chambaz for his feedback on an earlier draft that substantially improved our method and the paper}.
