...
\end{cases}
$$
mask counts in vector $\mathbf{y}$ whose posterior probability of belonging to any non-target spillover component is larger than to the target component, and re-estimate the distribution for the target marker with the newly masked counts,
$$
...
$$
\begin{aligned}
& P^{(1)}(Z_i = 1 \mid Y_i = 2)  = \frac{1/2 \cdot 1/6}{1/2 \cdot 1/6 + 1/2 \cdot 1/3} = 1/3 \qquad & P^{(1)}Z_i = 2 \mid Y_i = 2) = 1 - 1/3 = 2/3 \\
& P^{(1)}(Z_i = 1 \mid Y_i = 3)  = \frac{1/2 \cdot 2/6}{1/2 \cdot 2/6 + 1/2 \cdot 2/3} = 1/3 \qquad & P^{(1)}(Z_i = 2 \mid Y_i = 3) = 1 - 1/3 = 2/3 \\
...
$$
and mask (indicated by an $\times$) elements in vector $\mathbf{y}$ whose posterior probability of belonging to marker 2 is larger than to marker 1,
$$
...
\end{bmatrix}.
$$
Finally, re-estimate the distribution for the target marker and calculate the spillover probability estimate, 
$$
\begin{aligned}
