...
\textcolor{red}{
The original experiment from Figure \ref{spillr-vignette} is designed such that we expect no spillover in the channels on the vertical axis, but our diagnostic plots show that the bead distribution of at least one spillover marker---as identified by the spillover matrix in \texttt{CATALYST}---overlaps with the first mode in the distribution of real cells. Thus, the strong compensation by \texttt{spillR} is consistent with our assumption (A1). The results of \texttt{spillR (naive)} fit with this explanation as it considers all counts in that low count range as spillover, which occurs when there is only spillover and no target marker signal for these counts in the bead experiment.
}
