...
\textcolor{red}{
Our baseline method, \texttt{spillR (naive)}, illustrates the behavior of more aggressive compensation by considering only the bead distribution. If our baseline method compensates aggressively in a certain range, this is because most bead counts observed in that range are spillover counts. This approach highlights the allure and pitfalls of overcorrecting. While in Figure 1 it may seem that \texttt{spillR (naive)} compensates for all spillover by setting the first mode to zero (just like \texttt{CATALYST}), a closer inspection reveals that discrepancies between the bead spillover distribution and the first mode of the real cell distribution are not taken into account by either method, but do reflect in the compensation of \texttt{spillR}. A similar pattern can be seen in panels A, B, and C of Figure 3. This behavior reflects our assumption (A1) and highlights the role of bead experiments in the compensation process performed by \texttt{spillR}.
}

\textcolor{red}{
Our assumption (A1) is sometimes true and sometimes violated in the bead and cell data from \texttt{CATALYST}. In particular, in our leading example in Figure 1, we can see that the mode of the bead distribution for marker Yb173Di is very close to the mode in the cell data. So, for this marker our assumption is valid. There are other markers where we see a violation of this assumption.
}

\textcolor{red}{
The original experiment from Figure 3 is designed such that we expect no spillover in the channels on the vertical axis, but our diagnostic plots show that the bead distribution of at least one spillover marker---as identified by the spillover matrix in \texttt{CATALYST}---overlaps with the first mode in the distribution of real cells. Thus, the strong compensation by \texttt{spillR} is consistent with our assumption (A1). The results of \texttt{spillR (naive)} fit with this explanation as it considers all counts in that low count range as spillover, which occurs when there is only spillover and no target marker signal for these counts in the bead experiment.
}
