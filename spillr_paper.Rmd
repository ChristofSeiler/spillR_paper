...
Our data are count matrices that contain cells on the rows and markers on the columns. We have two count matrices, a real cells matrix and a beads matrix. In general, the number of beads is much smaller than real cells. To simplify notation, but without loss of generality, we will assume that both matrices have the same dimension $K \times J$. We denote an element from this matrix as $y_{kj}$. We count the frequency of cells that have $y$ proteins on marker $j$,
$$
y_{\cdot j} = \sum_{k = 1}^K I(y_{kj} = y),
$$ \textcolor{red}{TODO:} merge into nonparametric estimate?
where $I(\cdot)$ is the indicator function. 

First, we estimate the $P(Z_{j \to i} = y)$ on the beads experiment data. Second, we initialize $P^{(0)}(Z_{j \to i} = y)$ and $\pi^{(0)}$ using the real data. Third, we iterate the following E and M steps on the real data until convergence, here, only $P^{(t)}(Z_{j \to i} = y)$ and $\pi^{(t)}$ are being updated, while the spillover probabilities are kept as estimated from the beds experiment.

We initialize the probability mass function using the nonparametric estimate,
$$
P^{(0)}(Z_{j \to i} = y) = \frac{y_{\cdot j}}{\sum_{j = 1}^J y_{\cdot j}},
$$
and set the mixture probability vector to the discrete uniform, 
$$
